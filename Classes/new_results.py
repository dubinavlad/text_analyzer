...
class Dump:
    def __init__(self,agents,agents_numb,atribute,atribute_numb,verb,verb_numb,point,money,money_numb,data,person,uri,uri_number):
        self.agents = agents
    ...
    def to_result(self):
        ...
        for uri_index in reversed(self.uri_number):
                for agent_index in reversed(self.agents_numb):

                    if(uri_index>agent_index):
                        a_tmp=self.agents[self.agents_numb.index(agent_index)]+' '+self.uri[self.uri_number.index(uri_index)]
